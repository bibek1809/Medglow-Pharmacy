...
export default function AdminReportsTab({ reports, onRefresh, onError, onSuccess }: { reports: Report[]; onRefresh: () => void; onError: (message: string) => void; onSuccess: (message: string) => void }) {
  ...
  const importWorkbook = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch('/api/admin/reports/import', { method: 'POST', credentials: 'same-origin', body })
      const data = await response.json()
      if (!response.ok) throw new Error(data.error || 'Unable to import workbook')
      onRefresh(); onSuccess(`${data.imported} report rows imported successfully`)
    } catch (error) { onError(error instanceof Error ? error.message : 'Unable to import workbook') }
